...
namespace Classic
{
    public static class GetClassic
    {
        ...
        public static bool MyInit()
        {
            ...
            string d2dir = "c:\\games\\descent2";
            string hogFile = Path.Combine(dir, "descent.hog"), palFile = "palette.256";
            if (d2)
            {
                hogFile = Path.Combine(d2dir, "descent2.hog");
                palFile = "groupa.256";
            }
            hog = new Hog(hogFile);
            byte[] vgapal = hog.ItemData(palFile);
            ...
            if (d2)
                pigFile = Path.Combine(d2dir, "groupa.pig");
            ...
            if (d2)
            {
                pig.ReadD2Sound(Path.Combine(d2dir, "descent2.s11"));
                byte[] data = File.ReadAllBytes(Path.Combine(d2dir, "descent2.ham"));
                ClassicData = new ClassicData();
                ClassicData.Read(new BinaryReader(new MemoryStream(data, 8, data.Length - 8)), Version.D2);
            }
            else
        ...
        public static unsafe Texture2D MkTex(int bmidx)
        {
            int width, height;
        ...
        public static Material MkMat(int bmidx, int color)
        {
            Material mat;
            var key = bmidx >= 0 ? bmidx : -color;
            if (ClassicMats.TryGetValue(key, out mat))
                return mat;
            ...
            {
                Color c = d2 ? new Color32(bit5to8(color >> 10), bit5to8((color >> 5) & 31), bit5to8(color & 31), 255) : pal32[color];
                mat.SetColor("_Color", c);
                if (color != 0)
                {
                    mat.SetColor("_EmissionColor", c * .25f);
                    mat.EnableKeyword("_EMISSION");
                }
            }
            ClassicMats.Add(key, mat);
            return mat;
        }
        ...
        {
            CreateRobotModels();
            var go = UnityEngine.Object.Instantiate<GameObject>(ClassicRobots[n]);
            go.SetActive(true);
            return go;
        }
